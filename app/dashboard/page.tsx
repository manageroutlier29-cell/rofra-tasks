...
export default function Dashboard() {
  ...
  const [userName, setUserName] = useState('robertwaweru324');
  const [balance, setBalance] = useState(10.00);
  const [activeTab, setActiveTab] = useState<'dashboard' | 'earnings' | 'help'>('dashboard');
    ...
    <div className="min-h-screen bg-[#f1f1eb] text-slate-800 font-sans pb-24">
      ...
      <header className="bg-[#f1f1eb] px-4 py-3 flex justify-between items-center max-w-md mx-auto sticky top-0 z-30">
        ...
        >
          ...
          <span className="font-extrabold text-xs tracking-wider text-slate-900 uppercase">ROFRA TASKS</span>
        </div>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => alert('No new notifications')} 
            className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-xs shadow-xs hover:bg-slate-50"
          >
            🔔
          </button>
          ...
          <button 
            ...
            className="w-8 h-8 rounded-full bg-[#a3c99a] text-[#1c4d2e] font-bold text-xs flex items-center justify-center shadow-xs border border-white"
          >
        ...
        {activeTab === 'earnings' && (
          <div className="space-y-4 pt-2">
            <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
              <span>📊</span> Earnings Analytics
            </h2>

            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs">
                <span className="text-[10px] text-slate-400 font-bold block uppercase">Total Earned</span>
                <span className="text-xl font-black text-[#2a7a4c] mt-1 block">$10.00</span>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs">
                <span className="text-[10px] text-slate-400 font-bold block uppercase">Pending Payouts</span>
                <span className="text-xl font-black text-amber-600 mt-1 block">$0.00</span>
              </div>
            </div>

            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs space-y-3">
              <h3 className="text-xs font-bold text-slate-800">Completed Tasks History</h3>
              <div className="divide-y divide-slate-100 text-xs">
                <div className="py-2.5 flex justify-between items-center">
                  <div>
                    <p className="font-bold text-slate-800">Welcome Bonus Task</p>
                    <p className="text-[9px] text-slate-400">Completed • Free Access</p>
                  </div>
        ...
        {activeTab === 'help' && (
          <div className="space-y-4 pt-2">
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs space-y-3 text-xs">
              ...
              <div className="p-3 bg-[#f7f9f7] rounded-xl space-y-1">
                <p className="font-bold text-slate-900">How do I withdraw earnings?</p>
                <p className="text-slate-500 text-[11px]">Click the center Withdraw button or use the dashboard balance panel to initiate an M-Pesa payout.</p>
              </div>

              <div className="p-3 bg-[#f7f9f7] rounded-xl space-y-1">
                <p className="font-bold text-slate-900">Why upgrade to Pro?</p>
                <p className="text-slate-500 text-[11px]">Pro accounts remove withdrawal limits and grant access to high-paying micro-tasks.</p>
              </div>
            </div>

            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs text-center space-y-2">
              <p className="text-xs font-bold text-slate-800">Need live assistance?</p>
              <p className="text-[10px] text-slate-400">Our ROFRA TASKS support team is online 24/7.</p>
              <button 
                onClick={() => alert('Support request submitted. An agent will contact you.')}
                className="bg-[#2a7a4c] text-white text-xs font-bold px-4 py-2 rounded-xl transition"
              >
                Contact Support
              </button>
            </div>
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-5 max-w-xs w-full shadow-2xl border border-slate-100 space-y-4 text-center">
            <div className="w-16 h-16 rounded-full bg-[#a3c99a] text-[#1c4d2e] font-black text-2xl flex items-center justify-center mx-auto shadow-inner">
              {userName.charAt(0).toUpperCase()}
            </div>
            
            <div>
              <h3 className="font-black text-base text-slate-900 capitalize">{userName}</h3>
              <p className="text-xs text-slate-400">rofra-tasks User</p>
              <span className="inline-block mt-2 text-[10px] font-extrabold text-[#2a7a4c] bg-emerald-50 px-3 py-1 rounded-full">
                FREE MEMBER
              </span>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100">
              <button 
                onClick={handleNavigateToUpgrade}
                className="w-full bg-[#f28c28] text-white text-xs font-bold py-2.5 rounded-xl shadow-xs"
              >
                Upgrade to Pro
              </button>
              <button 
          ...
          <div className="bg-[#f4f7f4] rounded-3xl p-5 max-w-xs w-full shadow-2xl border border-slate-200 relative">
            ...
            {withdrawStep === 'details' && (
              <form onSubmit={handleProcessWithdrawal} className="space-y-3">
                ...
                <button 
                  type="submit" 
                  className="w-full bg-[#2a7a4c] text-white font-bold py-3 rounded-xl text-xs transition mt-2 shadow-xs"
                >
                  Process Withdrawal
                </button>
              </form>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200/80 px-6 py-2 z-40 flex justify-between items-center max-w-md mx-auto">
        <button 
          onClick={() => setActiveTab('dashboard')} 
          className={`flex flex-col items-center gap-0.5 ${activeTab === 'dashboard' ? 'text-[#2a7a4c]' : 'text-slate-400 hover:text-slate-700'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-0.5 ${activeTab === 'earnings' ? 'text-[#2a7a4c]' : 'text-slate-400 hover:text-slate-700'}`}
        >
          <span className="text-base">📊</span>
        ...
        <button 
          onClick={openWithdrawModal} 
          className="flex flex-col items-center gap-0.5 text-[#2a7a4c]"
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-0.5 ${activeTab === 'help' ? 'text-[#2a7a4c]' : 'text-slate-400 hover:text-slate-700'}`}
        >
          <span className="text-base">❓</span>
